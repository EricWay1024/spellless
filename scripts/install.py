...
def read_stamped_version(path: Path) -> tuple[str, str] | None:
    """`revision` and `built` out of a version.lua, when one has been written.

    The repository's copy says "source" and carries no build date, which is
    exactly the case this must decline: a checkout with git missing should
    still say it does not know, rather than claim to be a release.
    """
    try:
    ...
    built = re.search(r'built\s*=\s*"([^"]+)"', text)
    if not revision or revision.group(1) == "source":
        return None
    return revision.group(1), built.group(1) if built else "unknown"
...
def stamp_version(user_dir: Path, dry_run: bool) -> Path | None:
    ...
    """
    revision = git("describe", "--always", "--dirty", "--exclude", "*")
    built = git("log", "-1", "--format=%cs")
    if revision is None:
        # No git, which means this is a release archive rather than a checkout.
        # scripts/package.py already wrote the release version into the copy
        # being installed; keeping it beats overwriting a real answer with
        # "unknown" in the one file whose whole job is saying what is running.
        stamped = read_stamped_version(REPO / "rime" / "lua" / "spellless" / "version.lua")
        revision, built = stamped or ("unknown", "unknown")
    installed = datetime.now().strftime("%Y-%m-%d %H:%M")
    built = built or "unknown"
    text = (
...
# ---------------------------------------------------------------------------
# what to do next, which is different on every platform
# ---------------------------------------------------------------------------

# Redeploying is the step people miss, and naming the wrong menu is how they
# miss it: the copy is done, and only the frontend reloading makes it real.
REDEPLOY = {
    "Windows": ("Right-click the Weasel tray icon and choose the redeploy entry\n"
                "     (Chinese builds: 「重新部署」), or run WeaselDeployer.exe /deploy."),
    "Darwin": ("Click the Squirrel icon in the menu bar and choose Deploy\n"
               "     (「重新部署」), or run\n"
               "     \"/Library/Input Methods/Squirrel.app/Contents/MacOS/Squirrel\" --reload."),
    "Linux": ("Restart the input method: `ibus restart`, or for fcitx5\n"
              "     `fcitx5-remote -r`."),
}

LOGS = {
    "Windows": "%APPDATA%\\Rime\\rime.log, or the newest rime.*.log beside it",
    "Darwin": "~/Library/Rime/rime.log, or the newest rime.*.log beside it",
    "Linux": "the rime.log in the user directory above, or the newest rime.*.log",
}


def next_steps() -> str:
    system = platform.system()
    if system not in REDEPLOY:
        system = "Linux"
    return f"""
Next:
  1. {REDEPLOY[system]}
  2. Press F4 (or Control+grave) and pick "Spellless".
  3. Type `zzver` in any text box to confirm what is actually running.

Do not delete the `build` directory to force a rebuild: while it is gone, Rime
falls back to the schema list that ships with the frontend and asks you to
choose one.  This script marks the build stale instead.

If no candidates appear, look in {LOGS[system]}
for lines mentioning "spellless".  A missing librime-lua is the usual cause,
and it says so."""
